...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nom === '' || $preu === '' || $estoc === '') {
        $error = 'Els camps nom, preu i estoc son obligatoris.';

    } elseif (!is_numeric($preu) || $preu < 0) {
        $error = 'El preu ha de ser un numero positiu.';

    } elseif (!is_numeric($estoc) || $estoc < 0) {
        $error = 'L\'estoc ha de ser un numero positiu.';
...
<form method="POST" action="nou_producte.php" enctype="multipart/form-data" class="formulari-admin">
    ...
    <label for="imatge">Imatge (JPG, PNG, WEBP - max. 2MB)</label>
